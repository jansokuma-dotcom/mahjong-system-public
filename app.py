...
def load_data():
    ...
    # 2. すでにロード完了フラグが立っている場合は、メモリ（State）のデータをそのまま返す（高速化）
    ...
    if res.status_code == 200:
        ...
        st.session_state["github_sha"] = res_json.get("sha")  # 最新のファイル識別子を保管
...
def save_excel(df_g, df_m, df_l):
    ...
    # 3. 最新のsha（ファイル識別子）をGitHubから直接リアルタイム再取得（コンフリクトを完全防止）
    url = f"https://api.github.com/repos/{GITHUB_REPO}/contents/{FILE_PATH}"
    ...
    if not st.session_state["logged_in"]:
        ...
        with tab1:
            ...
            if not df_my_ranks.empty:
                ...
                fig_pie = px.pie(
                    rank_counts, values="回数", names="着順", hole=0.3, color="着順",
