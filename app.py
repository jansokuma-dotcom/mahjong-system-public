        ...
        if st.button("ログイン") and uid and upw:
            ...
            if not user.empty:
                ...
                new_l = pd.DataFrame(
                    [
                        {
                            "閲覧日時": datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S"),
                            "ログインID": uid,
                            "名前": uname,
                        }
                    ]
            ...
            if not df_games.empty:
                ...
                if "月間" in p_choice:
                    ...
                    if now.day <= 25:
                        ...
                        min_g = 30
                        st.caption(f"📢 月月末のランキング掲載条件: **30半荘以上**")
                else:
                ...
                if not df_f.empty:
                    stats = df_f.groupby("名前")["着順"].agg(対戦数="count", average_rank="mean").reset_index()
                    stats = stats.rename(columns={"average_rank": "平均着順"})
                    ...
                    if not ranking.empty:
                        ...
                        st.dataframe(
                            rk_sorted[["順位", "Web用表示名", "現在の段位", "平均着順", "対戦数", "現在のレート"]],
